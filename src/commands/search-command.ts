    ...
    if (!query) {
      const helpEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '💡 Ejemplos', value: '`!capellan buscar Emperador de la Humanidad`\n`!capellan buscar Space Marines`\n`!capellan buscar Warp`', inline: false }
        )
    ...
    try {
      ...
      if (result.sources.length > 0) {
        const sourcesText = result.sources
          .slice(0, 5)
          .map((source, index) => `${index + 1}. ${source.source} (${Math.round(source.similarity * 100)}% relevancia)`)
          .join('\n');
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setDescription('*Los espíritus de la máquina no responden. Los archivos sagrados están temporalmente inaccesibles.*')
