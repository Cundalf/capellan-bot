...
export class DocumentProcessor {
  ...
  async downloadAndProcessUrl(url: string, addedBy: string): Promise<void> {
    ...
    const metadataPath = join(this.config.documentsPath, 'metadata', `${fileName}_${Date.now()}_metadata.json`);
  ...
  private validateDomain(url: string): void {
    ...
    if (!isAllowed) {
      throw new Error(`Dominio no permitido: ${domain}. Solo se permiten: ${WARHAMMER_CONSTANTS.ALLOWED_DOMAINS.join(', ')}`);
    }
  }
  ...
  private async extractTextFromPdf(pdfPath: string): Promise<string> {
    try {
      ...
      // Clean up the text
      let text = this.cleanExtractedText(fullText);
  ...
  private extractTitleFromText(text: string): string {
    ...
    for (let i = 0; i < Math.min(5, lines.length); i++) {
      ...
      if (line.length >= 10 && line.length <= 100 && !line.includes('.') && !line.toLowerCase().includes('page')) {
        return line;
      }
    }
  ...
  async processTextDocument(text: string, source: string, addedBy: string, title?: string): Promise<void> {
